...
import { Bell, LogOut, Map as MapIcon, Menu, User, X, UtensilsCrossed, Sun, Moon, ChevronRight, Maximize2, Minimize2 } from 'lucide-react'
...
// Lazy-loaded so Leaflet/the map bundle only downloads when the Admin (dashboard) page is open,
// not on every route — it's only rendered in the sidebar while on /dashboard anyway.
const RestaurantMapCard = lazy(() => import('../features/dashboard/components/RestaurantMapCard'))
...
export default function AppLayout() {
  ...
  const [mapModalFullscreen, setMapModalFullscreen] = useState(false)
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-[#0B0F17] text-white border-r border-slate-800/70 select-none shadow-xl">
      ...
      {/* Restaurant Branches Map — shown for the ADMIN ROLE on every page (not tied to a
          route), sitting just above the account card. Compact icon-sized so it never
          covers the nav items for kitchen / cashier / etc. */}
      {user?.role === ROLES.ADMIN && (
        <div className="px-4 pb-1 pt-2">
          <button
            ...
            aria-label={t('dashboard.branchesMap', { defaultValue: 'Restoranlar xaritasi' })}
            className="flex size-8 items-center justify-center rounded-lg bg-orange-500/10 text-orange-400 transition-all hover:bg-orange-500/20 hover:text-orange-300"
          >
            <MapIcon size={15} />
          </button>
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F17] transition-colors">
      ...
      {/* Restaurant Branches Map — opens centered, ~half the viewport, over the current page */}
      <Modal
        isOpen={mapModalOpen}
        onClose={() => setMapModalOpen(false)}
        title={t('dashboard.branchesMap', { defaultValue: 'Restoranlar xaritasi' })}
        className="w-[98vw] max-w-none h-[96vh] max-h-none min-w-[320px] min-h-[420px]"
