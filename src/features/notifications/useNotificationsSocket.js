...
export default function useNotificationsSocket() {
  ...
  useEffect(() => {
    ...
    const handleOrderReady = (payload) => {
      ...
      // Shared toastId with useKitchenOrders.js's 'complete' toast + handleNotificationNew below,
      // so all 3 near-simultaneous "order ready" triggers collapse into a single visible toast.
      toast.success(notification.message, { toastId: `order-ready-${orderId}` })
    }
    ...
    const handleNotificationNew = (payload) => {
      ...
      if (notification.message) {
        // If this generic notification is about an order (e.g. also fired for "order ready"),
        // reuse the same toastId pattern so it collapses with the other two triggers instead
        // of showing a 3rd, near-identical toast.
        const toastId = notification.orderId ? `order-ready-${notification.orderId}` : `notif-${notifId}`
