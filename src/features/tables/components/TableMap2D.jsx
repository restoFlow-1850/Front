          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-1 sm:pb-0">
            {zoneNames.map((z) => (
              <button
                ...
                className={`min-h-11 shrink-0 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] transition ${zoneFilter === z ? 'bg-orange-600 text-white dark:bg-cyan-500 dark:text-slate-950' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-[#04111a] dark:text-slate-300 dark:hover:bg-[#071f30]'}`}
              >
                {z}
              </button>
            ))}
      ...
      {!pickerMode && <div className="mt-2 flex gap-1.5">
        ...
        {canTransfer && (
          <button
            ...
            className="inline-flex min-h-11 items-center gap-1 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[10px] uppercase tracking-[0.16em] text-slate-500 transition hover:border-orange-300 hover:text-orange-600 dark:border-cyan-500/10 dark:bg-[#08111f] dark:text-cyan-200 dark:hover:border-cyan-400 dark:hover:text-white"
          >
