...
export function useKitchenOrders() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (_data, { id, columnKey }) => {
      // Same toastId as the global "order ready" toast (useNotificationsSocket.js) for the
      // 'complete' column, so react-toastify collapses the near-simultaneous duplicate into one.
      const toastId = columnKey === 'complete' ? `order-ready-${id}` : `order-status-${id}-${columnKey}`
      toast.success(t('kitchen.statusChanged', { status: t(`kitchen.columns.${columnKey}`) }), { toastId })
    },
    onSettled: () => {
      invalidateOrders()
    },
